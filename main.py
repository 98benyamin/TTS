    ...
    try:
        ...
        if update.message:
            await update.message.reply_text(
                ...
                "• داستان‌های چند شخصیتی با صداهای متفاوت بسازید\n\n"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اضافه کردن مدیریت کلیک روی دکمه "ساخت داستان پیشرفته" به message_handler
    if text == "📖 ساخت داستان پیشرفته":
        try:
            ...
            await update.message.reply_text(
                ...
                "با این قابلیت می‌توانید یک داستان چند شخصیتی بسازید که هر شخصیت صدا و حس متفاوتی دارد.\n\n"
                "روند کار به این صورت است:\n"
                "1️⃣ انتخاب قالب صوتی\n"
                "2️⃣ افزودن شخصیت‌ها (نام، صدا و حس هر شخصیت)\n"
                "3️⃣ نوشتن متن برای هر شخصیت\n"
                "4️⃣ دریافت فایل صوتی یکپارچه از داستان\n\n"
                "لطفاً ابتدا قالب صوتی نهایی را انتخاب کنید:",
                reply_markup=reply_markup,
                parse_mode="HTML"
            )
            context.user_data["state"] = "story_select_format"
            context.user_data["previous_state"] = "main"
            return None
        except Exception as e:
            logger.error(f"خطا در نمایش منوی ساخت داستان برای کاربر {user_id}: {str(e)}")
            return None

    # مدیریت وارد کردن نام شخصیت
    elif context.user_data.get("state") == "story_add_character":
        try:
            if text == "✅ پایان تعریف شخصیت‌ها":
                # اگر حداقل یک شخصیت تعریف شده باشد
                if context.user_data.get("story_characters", []):
                    # رفتن به مرحله بعد - انتخاب صدا برای شخصیت اول
                    context.user_data["state"] = "story_select_voice"
                    context.user_data["previous_state"] = "story_add_character"
                    context.user_data["current_character_index"] = 0
                    current_character = context.user_data["story_characters"][0]
                    ...
                    await update.message.reply_text(
                        f"🎙 <b>انتخاب صدا برای شخصیت '{current_character['name']}'</b>\n\n"
                        "لطفاً یکی از صداهای زیر را انتخاب کنید:",
                        reply_markup=reply_markup,
                        parse_mode="HTML"
                    )
                    return None
                else:
                    # اگر هیچ شخصیتی تعریف نشده باشد
                    await update.message.reply_text(
                        "❌ لطفاً حداقل یک شخصیت تعریف کنید.",
                        reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
                    )
                    return None
            
            # بررسی تعداد شخصیت‌های تعریف شده
            characters = context.user_data.get("story_characters", [])
            
            if len(characters) >= MAX_CHARACTERS:
                await update.message.reply_text(
                    f"❌ شما به محدودیت حداکثر {MAX_CHARACTERS} شخصیت رسیده‌اید.\n"
                    "لطفاً روی دکمه «پایان تعریف شخصیت‌ها» کلیک کنید.",
                    reply_markup=ReplyKeyboardMarkup([["✅ پایان تعریف شخصیت‌ها"], ["🔙 برگشت"]], resize_keyboard=True)
                )
                return None
                
            # اضافه کردن شخصیت جدید
            character_name = text.strip()
            
            # بررسی طول نام
            if len(character_name) > 30:
                await update.message.reply_text(
                    "❌ نام شخصیت باید کمتر از 30 کاراکتر باشد.",
                    reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
                )
                return None
            ...
            for char in characters:
                if char["name"].lower() == character_name.lower():
                    await update.message.reply_text(
                        "❌ این نام شخصیت قبلاً استفاده شده است. لطفاً نام دیگری انتخاب کنید.",
                        reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
                    )
                    return None
                    
            # افزودن شخصیت جدید به لیست
            context.user_data["story_characters"].append({"name": character_name})
            
            # تعیین گزینه‌های کیبورد بر اساس تعداد شخصیت‌ها
            if len(characters) + 1 >= MAX_CHARACTERS:
                keyboard = [["✅ پایان تعریف شخصیت‌ها"], ["🔙 برگشت"]]
            else:
                keyboard = [["✅ پایان تعریف شخصیت‌ها"], ["🔙 برگشت"]]
            ...
            await update.message.reply_text(
                f"✅ شخصیت '{character_name}' با موفقیت اضافه شد.\n\n"
                f"تعداد شخصیت‌های تعریف شده: {len(characters) + 1} از {MAX_CHARACTERS}\n\n"
                "لطفاً نام شخصیت بعدی را وارد کنید یا روی دکمه «پایان تعریف شخصیت‌ها» کلیک کنید:",
                reply_markup=reply_markup
            )
            return None
        except Exception as e:
            logger.error(f"خطا در افزودن شخصیت برای کاربر {user_id}: {str(e)}")
            await update.message.reply_text(
                "❌ خطا در افزودن شخصیت. لطفاً دوباره تلاش کنید.",
                reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
            )
            return None

    # اضافه کردن مرحله ورود متن برای داستان
    elif context.user_data.get("state") == "story_enter_text":
        try:
            # بررسی انتخاب شخصیت جدید
            if text.startswith("🎭 شخصیت: "):
                character_name = text[10:]  # حذف "🎭 شخصیت: " از ابتدای متن
                ...
                for idx, char in enumerate(context.user_data["story_characters"]):
                    if char["name"] == character_name:
                        ...
                        context.user_data["current_segment"] = {
                            "character_index": idx,
                ...
                if found:
                    await update.message.reply_text(
                        f"✅ شخصیت '{character_name}' انتخاب شد. لطفاً متن مربوط به این شخصیت را وارد کنید.",
                        reply_markup=ReplyKeyboardMarkup([["✅ پایان ورود متن‌ها"], ["🔙 برگشت"]], resize_keyboard=True)
                    )
                    return None
                else:
            ...
            # پایان ورود متن‌ها
            if text == "✅ پایان ورود متن‌ها":
                # بررسی تعداد قطعات داستان
                if not context.user_data.get("story_segments", []):
                    await update.message.reply_text(
                        "❌ شما هنوز هیچ متنی وارد نکرده‌اید. لطفاً حداقل یک متن را وارد کنید.",
                        reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
                    )
                ...
                # شروع فرآیند تولید صدا
                context.user_data["state"] = "story_processing"
                context.user_data["previous_state"] = "story_enter_text"
                
                audio_format = "mp3"  # فرمت پیش‌فرض
                
                # نمایش پیام با اطلاعات صداهای در حال تولید
                total_segments = len(context.user_data["story_segments"])
                
                initial_text = f"🔊 <b>در حال ساخت داستان با {total_segments} قطعه...</b>"
                ...
                # تبدیل هر قطعه داستان به صدا
                for i, segment in enumerate(context.user_data["story_segments"]):
                    character_idx = segment["character_index"]
                    character = context.user_data["story_characters"][character_idx]
                    
                    # اطلاعات شخصیت
                    voice = character["voice"]
                    voice_persian = character["voice_persian"]
                    feeling = character["feeling"]
                    feeling_name = character["feeling_name"]
                    character_name = character["name"]
                    segment_text = segment["text"]
                    
                    # ایجاد شناسه یکتا برای این درخواست تولید صدا
                    task_id = f"story_tts_{uuid4().hex}"
                    output_file = f"story_segment_{i}_{uuid4()}.{audio_format}"
                    API_TASKS[task_id] = {"status": "running", "result": None}
                    
                    # شروع درخواست API برای تولید صدا در یک ترد جداگانه
                    thread = threading.Thread(
                        target=run_api_task,
                        args=(task_id, generate_audio, segment_text, feeling, voice, output_file, audio_format)
                    )
                    thread.start()
                    ...
                    try:
                        # بروزرسانی پیشرفت
                        progress_text = f"🔊 <b>در حال ساخت داستان...</b>\n\n• <b>شخصیت:</b> {character_name}\n• <b>صدا:</b> {voice_persian}\n• <b>حس:</b> {feeling_name}\n\n⏳ <b>پیشرفت:</b> ({i+1}/{total_segments})"
                        await progress_message.edit_text(
                            progress_text,
                            reply_markup=keyboard,
                            parse_mode="HTML"
                ...
                # ادغام فایل‌های صوتی اگر تولید موفق بوده است
                if success_count > 0:
                    await update.message.reply_text(
                        f"✅ <b>{success_count} از {total_segments} قطعه داستان با موفقیت تولید شد!</b>\n"
                        "در حال ادغام فایل‌های صوتی...",
                        parse_mode="HTML"
                    )
                    ...
                    if merge_success:
                        ...
                        caption = "🎙 <b>داستان پیشرفته</b>\n\n<b>شخصیت‌ها:</b>\n"
                        for char in context.user_data["story_characters"]:
                        ...
                        with open(final_output_file, "rb") as audio:
                            await update.message.reply_audio(
                                audio=audio,
                                caption=caption,
                                title=f"داستان پیشرفته - {success_count} قطعه",
                                parse_mode="HTML"
                            )
                        ...
                        await update.message.reply_text(
                            "❌ خطا در ادغام فایل‌های صوتی. متأسفانه نمی‌توان داستان کامل را ارائه داد.",
                            parse_mode="HTML"
                        )
                        ...
                        for i, audio_file in enumerate(generated_audio_files):
                            try:
                                segment = context.user_data["story_segments"][i]
                                character_idx = segment["character_index"]
                                character = context.user_data["story_characters"][character_idx]
                                ...
                                with open(audio_file, "rb") as audio:
                                    await update.message.reply_audio(
                                        audio=audio,
                                        caption=f"🎙 <b>قطعه {i+1}</b>\n\n• <b>شخصیت:</b> {character['name']}\n• <b>صدا:</b> {character['voice_persian']}\n• <b>حس:</b> {character['feeling_name']}",
                                        title=f"قطعه {i+1} - {character['name']}",
                                        parse_mode="HTML"
                                    )
                                ...
                                logger.error(f"خطا در ارسال فایل صوتی قطعه {i+1}: {str(e)}")
                
                # نمایش قطعاتی که تبدیل نشده‌اند
                if failed_segments:
                    failed_message = "❌ <b>قطعات زیر تبدیل نشدند:</b>\n\n"
                    for i, failed in enumerate(failed_segments):
                        failed_message += f"{i+1}. شخصیت {failed['character_name']}: {failed['text']}\n"
                    await update.message.reply_text(
                        failed_message,
                        parse_mode="HTML"
                    )
                
                # پایان فرآیند و بازگشت به صفحه اصلی
                keyboard = [
                    ["🎙 تبدیل متن به صدا", "🤖 دستیار هوشمند"], 
                    ["🔊 نمونه صدا و حس ها", "📑 پردازش دسته‌ای"],
                    ["📖 ساخت داستان پیشرفته"]
                ...
                await update.message.reply_text(
                    "✅ <b>فرآیند ساخت داستان به پایان رسید!</b>\n\n"
                ...
                # حذف درخواست از صف
                if "request_id" in context.user_data:
                    await remove_from_queue(user_id, context.user_data["request_id"])
            ...
            # بررسی محدودیت تعداد قطعات
            if len(context.user_data.get("story_segments", [])) >= MAX_STORY_SEGMENTS:
                await update.message.reply_text(
                    f"❌ شما به محدودیت حداکثر {MAX_STORY_SEGMENTS} قطعه متن رسیده‌اید.\n"
                    "لطفاً روی دکمه «پایان ورود متن‌ها» کلیک کنید.",
                    reply_markup=ReplyKeyboardMarkup([["✅ پایان ورود متن‌ها"], ["🔙 برگشت"]], resize_keyboard=True)
                )
                return None
                
            # اضافه کردن متن به سگمنت فعلی
            current_segment = context.user_data["current_segment"].copy()
            current_segment["text"] = text
            
            # اضافه کردن به لیست قطعات
            if "story_segments" not in context.user_data:
                context.user_data["story_segments"] = []
                
            context.user_data["story_segments"].append(current_segment)
            
            # به روز رسانی وضعیت
            segment_count = len(context.user_data["story_segments"])
            character_name = current_segment["character_name"]
            
            # ساخت دکمه‌های شخصیت‌ها
            keyboard = [[f"🎭 شخصیت: {char['name']}" for char in context.user_data["story_characters"]]]
            keyboard.append(["✅ پایان ورود متن‌ها"])
            keyboard.append(["🔙 برگشت"])
            reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
            
            await update.message.reply_text(
                f"✅ متن شماره {segment_count} برای شخصیت '{character_name}' با موفقیت ثبت شد.\n\n"
                f"تعداد قطعات ثبت شده: {segment_count} از {MAX_STORY_SEGMENTS}\n\n"
                "لطفاً شخصیت بعدی را انتخاب کنید و متن مربوط به آن را وارد کنید یا\n"
                "روی دکمه «پایان ورود متن‌ها» کلیک کنید:",
                reply_markup=reply_markup
            )
            return None
        except Exception as e:
            logger.error(f"خطا در ورود متن برای کاربر {user_id}: {str(e)}")
            await update.message.reply_text(
                "❌ خطا در ورود متن. لطفاً دوباره تلاش کنید.",
                reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
            )
            return None

    # مدیریت انتخاب صدا برای شخصیت
    elif context.user_data.get("state") == "story_select_voice":
        try:
            voice_persian = text
            
            # بررسی معتبر بودن صدا
            if voice_persian in PERSIAN_TO_ORIGINAL_VOICE:
                voice = PERSIAN_TO_ORIGINAL_VOICE[voice_persian]
                current_index = context.user_data["current_character_index"]
                
                # ذخیره صدای انتخاب شده برای شخصیت فعلی
                context.user_data["story_characters"][current_index]["voice"] = voice
                context.user_data["story_characters"][current_index]["voice_persian"] = voice_persian
                
                # رفتن به مرحله انتخاب حس
                context.user_data["state"] = "story_select_tone_category"
                context.user_data["previous_state"] = "story_select_voice"
                ...
                character_name = context.user_data["story_characters"][current_index]["name"]
                await update.message.reply_text(
                    f"🎭 <b>انتخاب حس و لحن برای شخصیت '{character_name}'</b>\n\n"
                    f"صدای انتخاب شده: {voice_persian}\n\n"
                    "لطفاً دسته‌بندی حس و لحن را انتخاب کنید:",
                    reply_markup=reply_markup,
                    parse_mode="HTML"
                )
                return None
            ...
            logger.error(f"خطا در انتخاب صدا برای کاربر {user_id}: {str(e)}")
            await update.message.reply_text(
                "❌ خطا در انتخاب صدا. لطفاً دوباره تلاش کنید.",
                reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
            )
            return None
            
    # مدیریت انتخاب دسته‌بندی حس
    elif context.user_data.get("state") == "story_select_tone_category":
        try:
            category_map = {
                "✍️ لحن و حس دستی": "manual_feeling",
                "📢 لحن‌های کاربردی": "functional",
            ...
            if text in category_map:
                ...
                if text == "✍️ لحن و حس دستی":
                    ...
                    await update.message.reply_text(
                        f"✍️ <b>حس دستی برای شخصیت '{character_name}'</b>\n\n"
                    ...
                    category = category_map[text]
                    context.user_data["story_characters"][current_index]["tone_category"] = category
                    ...
                    await update.message.reply_text(
                        f"🎭 <b>{category_names[category]} برای شخصیت '{character_name}'</b>\n\n"
                        "لطفاً یکی از حس‌های زیر را انتخاب کنید:",
                        reply_markup=reply_markup,
                        parse_mode="HTML"
                    )
            ...
            logger.error(f"خطا در انتخاب دسته‌بندی حس برای کاربر {user_id}: {str(e)}")
            await update.message.reply_text(
                "❌ خطا در انتخاب دسته‌بندی حس. لطفاً دوباره تلاش کنید.",
                reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
            )
            return None
            
    # مدیریت ورود حس دستی
    elif context.user_data.get("state") == "story_manual_feeling":
        try:
            feeling = text
            if len(feeling) > MAX_FEELING_LENGTH:
                await update.message.reply_text(
            ...
            current_index = context.user_data["current_character_index"]
            character = context.user_data["story_characters"][current_index]
            
            # ذخیره حس دستی
            character["feeling"] = feeling
            character["feeling_name"] = "دستی"
            
            # مرحله بعد: وارد کردن متن
            context.user_data["state"] = "story_enter_text"
            context.user_data["previous_state"] = "story_manual_feeling"
            ...
            await update.message.reply_text(
                f"📝 <b>ورود متن برای شخصیت '{character['name']}'</b>\n\n"
                "اکنون لطفاً متن مربوط به این شخصیت را وارد کنید (حداکثر 1000 کاراکتر).\n\n"
                "پس از ورود متن، می‌توانید شخصیت دیگری را انتخاب کنید یا به پایان برسانید.",
                reply_markup=reply_markup,
                parse_mode="HTML"
            )
            
            # ایجاد یک سگمنت جدید با شخصیت فعلی
            context.user_data["current_segment"] = {
                "character_index": current_index,
                "character_name": character["name"]
            }
            
            return None
        except Exception as e:
            logger.error(f"خطا در ورود حس دستی برای کاربر {user_id}: {str(e)}")
            await update.message.reply_text(
                "❌ خطا در ورود حس دستی. لطفاً دوباره تلاش کنید.",
                reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
            )
            return None
            
    # مدیریت انتخاب حس از لیست
    elif context.user_data.get("state") == "story_select_tone":
        try:
            current_index = context.user_data["current_character_index"]
            character = context.user_data["story_characters"][current_index]
            category = character["tone_category"]
            tones = TONES[category]
            ...
            if selected_tone:
                # ذخیره حس انتخاب شده
                character["feeling"] = selected_tone["prompt"]
                character["feeling_name"] = selected_tone["name"]
                
                # مرحله بعد: وارد کردن متن
                context.user_data["state"] = "story_enter_text"
                context.user_data["previous_state"] = "story_select_tone"
            ...
            logger.error(f"خطا در انتخاب حس برای کاربر {user_id}: {str(e)}")
            await update.message.reply_text(
                "❌ خطا در انتخاب حس. لطفاً دوباره تلاش کنید.",
                reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
            )
            return None

    # مدیریت اضافه کردن شخصیت جدید
    elif context.user_data.get("state") == "story_add_character":
        try:
            character_name = text
            ...
            if len(character_name) > 50:
                await update.message.reply_text(
                    "❌ نام شخصیت نمی‌تواند بیشتر از 50 کاراکتر باشد. لطفاً نام کوتاه‌تری وارد کنید.",
                    reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
                )
                return None
                
            # اضافه کردن شخصیت جدید
            if "story_characters" not in context.user_data:
                context.user_data["story_characters"] = []
            ...
            context.user_data["story_characters"].append(new_character)
            current_index = len(context.user_data["story_characters"]) - 1
            context.user_data["current_character_index"] = current_index
            
            # رفتن به مرحله انتخاب صدا
            context.user_data["state"] = "story_select_voice"
            context.user_data["previous_state"] = "story_add_character"
            
            # ساخت دکمه‌های صدا
            keyboard = []
            voices_persian = list(PERSIAN_TO_ORIGINAL_VOICE.keys())
            for i in range(0, len(voices_persian), 2):
                row = [voices_persian[i]]
                if i + 1 < len(voices_persian):
                    row.append(voices_persian[i+1])
                keyboard.append(row)
            keyboard.append(["🔙 برگشت"])
            reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
            
            await update.message.reply_text(
                f"🔊 <b>انتخاب صدا برای شخصیت '{character_name}'</b>\n\n"
                "لطفاً یکی از صداهای زیر را انتخاب کنید:",
                reply_markup=reply_markup,
                parse_mode="HTML"
            )
    ...
    # مدیریت انتخاب قالب صوتی
    elif context.user_data.get("state") == "story_select_format":
        try:
            # بررسی اعتبار قالب صوتی
            ...
            # ذخیره قالب صوتی
            context.user_data["audio_format"] = text
            
            # آماده‌سازی برای اضافه کردن اولین شخصیت
            ...
            await update.message.reply_text(
                ...
                "لطفاً نام شخصیت اول را وارد کنید:",
                reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True),
                parse_mode="HTML"
            )
            ...
            logger.error(f"خطا در انتخاب قالب صوتی برای کاربر {user_id}: {str(e)}")
            await update.message.reply_text(
                "❌ خطا در انتخاب قالب صوتی. لطفاً دوباره تلاش کنید.",
                reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
            )
            return None

    # حالت‌های ساخت داستان
    if current_state == "story_select_format":
        return await start(update, context)
        
    if current_state == "story_add_character":
        keyboard = [["MP3", "WAV", "OGG"], ["🔙 برگشت"]]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        await update.message.reply_text(
            "📖 <b>ساخت داستان پیشرفته</b>\n\n"
    ...
    if current_state == "story_select_voice":
        context.user_data["story_characters"].pop()  # حذف شخصیت فعلی
        keyboard = [["🔙 برگشت"]]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        await update.message.reply_text(
            "🎭 <b>افزودن شخصیت جدید</b>\n\n"
    ...
    if current_state == "story_select_tone_category":
        keyboard = []
        voices_persian = list(PERSIAN_TO_ORIGINAL_VOICE.keys())
        for i in range(0, len(voices_persian), 2):
            row = [voices_persian[i]]
    ...
    if current_state == "story_select_tone":
        keyboard = [
            ["✍️ لحن و حس دستی"],
            ["📢 لحن‌های کاربردی", "👑 لحن‌های نمایشی / شخصیتی"],
            ["🎤 لحن‌های گفتاری", "🎭 لحن‌های احساسی"],
    ...
    if current_state == "story_manual_feeling":
        keyboard = [
            ["✍️ لحن و حس دستی"],
            ["📢 لحن‌های کاربردی", "👑 لحن‌های نمایشی / شخصیتی"],
            ["🎤 لحن‌های گفتاری", "🎭 لحن‌های احساسی"],
    ...
    if current_state == "story_enter_text":
        current_index = context.user_data["current_character_index"]
        character = context.user_data["story_characters"][current_index]
        
        if "feeling_name" in character:
            # برگشت به مرحله انتخاب حس
            if character.get("feeling_name") == "دستی":
                await update.message.reply_text(
                    f"✍️ <b>حس دستی برای شخصیت '{character['name']}'</b>\n\n"
                    "لطفاً حس یا دستورالعمل‌های صدا را وارد کنید (حداکثر 500 کاراکتر).\n"
                    "مثال: Dramatic یا Gruff, fast-talking, New York accent",
                    reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True),
                    parse_mode="HTML"
                ...
                category = character.get("tone_category", "emotional")
                tones = TONES[category]
                keyboard = []
                for i in range(0, len(tones), 2):
                    row = [f"{tones[i]['emoji']} {tones[i]['name']}"]
                ...
                await update.message.reply_text(
                    f"🎭 <b>{category_names[category]} برای شخصیت '{character['name']}'</b>\n\n"
                    "لطفاً یکی از حس‌های زیر را انتخاب کنید:",
                    reply_markup=reply_markup,
                    parse_mode="HTML"
                )
            ...
            await update.message.reply_text(
                f"🔊 <b>انتخاب صدا برای شخصیت '{character['name']}'</b>\n\n"
                "لطفاً یکی از صداهای زیر را انتخاب کنید:",
                reply_markup=reply_markup,
                parse_mode="HTML"
            )
    ...
    # مدیریت ورود متن برای شخصیت‌ها در حالت داستان
    elif context.user_data.get("state") == "story_enter_text":
        try:
            # بررسی پایان ورود متن‌ها
            if text == "✅ پایان ورود متن‌ها":
                # بررسی وجود حداقل یک سگمنت
                if not context.user_data.get("story_segments"):
                    await update.message.reply_text(
                        "❌ لطفاً قبل از پایان ورود متن‌ها، حداقل یک متن برای یکی از شخصیت‌ها وارد کنید.",
                        reply_markup=ReplyKeyboardMarkup([[f"🎭 شخصیت: {char['name']}" for char in context.user_data["story_characters"]][:2], 
                                                      ["✅ پایان ورود متن‌ها"],
                ...
                # شروع فرآیند تولید صدا
                progress_message = await update.message.reply_text(
                    "⏳ در حال تولید داستان صوتی... این فرآیند ممکن است چند دقیقه طول بکشد.",
                    reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
                )
                
                # ذخیره پیام در وضعیت برای به‌روزرسانی پیشرفت
                context.user_data["progress_message"] = progress_message
                context.user_data["progress_chat_id"] = update.message.chat_id
                ...
                audio_files = []
                failed_segments = []
                segments = context.user_data["story_segments"]
                
                # به‌روزرسانی پیام پیشرفت
                await context.bot.edit_message_text(
                    f"⏳ در حال تولید داستان صوتی...\n\n0/{len(segments)} قطعه تکمیل شده",
                    chat_id=update.message.chat_id,
                    message_id=progress_message.message_id
                )
                
                # تولید فایل صوتی برای هر سگمنت
                for i, segment in enumerate(segments):
                    try:
                        character_index = segment["character_index"]
                        character = context.user_data["story_characters"][character_index]
                    ...
                    # به‌روزرسانی پیام پیشرفت
                    await context.bot.edit_message_text(
                        f"⏳ در حال تولید داستان صوتی...\n\n{i+1}/{len(segments)} قطعه تکمیل شده",
                        chat_id=update.message.chat_id,
                        message_id=progress_message.message_id
                    )
                ...
                if audio_files:
                    try:
                        final_audio = await merge_audio_files([file["file"] for file in audio_files], audio_format, user_id)
                        
                        # ارسال فایل صوتی نهایی
                        caption = "🎙 <b>داستان صوتی شما آماده است!</b>\n\n"
                        caption += "شامل دیالوگ‌های:\n"
                        for file in audio_files:
                            caption += f"🎭 <b>{file['character']}:</b> {file['text']}\n"
                        
                        if failed_segments:
                            caption += "\n⚠️ <b>سگمنت‌های ناموفق:</b>\n"
                            for segment in failed_segments:
                                caption += f"🎭 <b>{segment['character']}:</b> {segment['text']}\n"
                        
                        # ارسال فایل صوتی با توضیحات
                        with open(final_audio, "rb") as audio:
                            if audio_format.lower() == "mp3":
                                await context.bot.send_audio(
                                    ...
                                    caption=caption,
                                    parse_mode="HTML",
                                    title=f"داستان صوتی - {get_current_time_str()}"
                                )
                            else:
                        ...
                        logger.error(f"خطا در ادغام فایل‌های صوتی برای کاربر {user_id}: {str(e)}")
                        await update.message.reply_text(
                            "❌ متأسفانه خطایی در ادغام فایل‌های صوتی رخ داد.\n"
                            "در حال ارسال فایل‌های صوتی به صورت جداگانه...",
                            reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
                        )
                        
                        # ارسال فایل‌های صوتی به صورت جداگانه
                        for file in audio_files:
                            try:
                                caption = f"🎭 <b>{file['character']}:</b> {file['text']}"
                                with open(file["file"], "rb") as audio:
                                    if audio_format.lower() == "mp3":
                                        await context.bot.send_audio(
                                            chat_id=update.message.chat_id,
                                            audio=audio,
                                            caption=caption,
                                            parse_mode="HTML",
                                            title=f"{file['character']} - {get_current_time_str()}"
                                        )
                                    else:
                                        await context.bot.send_voice(
                                            chat_id=update.message.chat_id,
                    ...
                    await update.message.reply_text(
                        "❌ متأسفانه هیچ یک از سگمنت‌ها با موفقیت تبدیل به صدا نشدند.\n"
                ...
                await update.message.reply_text(
                    "✅ <b>فرآیند ساخت داستان به پایان رسید!</b>\n\n"
                    "برای تولید داستان جدید یا استفاده از سایر امکانات، یکی از دکمه‌های زیر را انتخاب کنید:",
                    reply_markup=reply_markup,
                    parse_mode="HTML"
                )
                context.user_data.clear()
            ...
            # انتخاب شخصیت
            for character_str in [f"🎭 شخصیت: {char['name']}" for char in context.user_data["story_characters"]]:
                if text == character_str:
                    character_name = text.replace("🎭 شخصیت: ", "")
                    
                    # یافتن شخصیت در لیست
                    found = False
                    for i, char in enumerate(context.user_data["story_characters"]):
                        if char["name"] == character_name:
                            context.user_data["current_character_index"] = i
                            found = True
                            break
                    
                    if found:
                        character = context.user_data["story_characters"][context.user_data["current_character_index"]]
                        
                        await update.message.reply_text(
                            f"📝 <b>ورود متن برای شخصیت '{character_name}'</b>\n\n"
                            f"صدا: {character.get('voice_persian', 'نامشخص')}\n"
                            f"حس: {character.get('feeling_name', 'نامشخص')}\n\n"
                            "لطفاً متن مربوط به این شخصیت را وارد کنید (حداکثر 1000 کاراکتر):",
                            reply_markup=ReplyKeyboardMarkup([[f"🎭 شخصیت: {char['name']}" for char in context.user_data["story_characters"]][:2], 
                                                          ["✅ پایان ورود متن‌ها"],
                                                          ["🔙 برگشت"]], resize_keyboard=True),
                            parse_mode="HTML"
                        ...
                        # تنظیم شخصیت فعلی برای دریافت متن
                        context.user_data["current_segment"] = {
                            "character_index": context.user_data["current_character_index"],
                            "character_name": character_name
                        }
                        return None
                    else:
                        await update.message.reply_text(
                            "❌ شخصیت مورد نظر یافت نشد. لطفاً از لیست انتخاب کنید.",
                            reply_markup=ReplyKeyboardMarkup([[f"🎭 شخصیت: {char['name']}" for char in context.user_data["story_characters"]][:2], 
                                                          ["✅ پایان ورود متن‌ها"],
                                                          ["🔙 برگشت"]], resize_keyboard=True)
                        )
            ...
            # دریافت متن برای شخصیت فعلی
            if "current_segment" in context.user_data:
                text_input = text
                
                # بررسی طول متن
                if len(text_input) > MAX_TEXT_LENGTH:
                    await update.message.reply_text(
                        f"❌ خطا: متن شما {len(text_input)} کاراکتر است. لطفاً متنی با حداکثر {MAX_TEXT_LENGTH} کاراکتر وارد کنید.",
                        reply_markup=ReplyKeyboardMarkup([[f"🎭 شخصیت: {char['name']}" for char in context.user_data["story_characters"]][:2], 
                                                      ["✅ پایان ورود متن‌ها"],
                                                      ["🔙 برگشت"]], resize_keyboard=True)
                    )
                ...
                # ذخیره متن در سگمنت فعلی
                current_segment = context.user_data["current_segment"]
                current_segment["text"] = text_input
                
                # اضافه کردن به لیست سگمنت‌ها
                if "story_segments" not in context.user_data:
                    context.user_data["story_segments"] = []
                ...
                character_name = current_segment["character_name"]
                await update.message.reply_text(
                    f"✅ متن برای شخصیت «{character_name}» ثبت شد.\n\n"
                    "لطفاً شخصیت دیگری را انتخاب کنید یا روی دکمه «پایان ورود متن‌ها» کلیک کنید.",
                    reply_markup=ReplyKeyboardMarkup([[f"🎭 شخصیت: {char['name']}" for char in context.user_data["story_characters"]][:2], 
                                                  ["✅ پایان ورود متن‌ها"],
                                                  ["🔙 برگشت"]], resize_keyboard=True)
                )
                
                # آماده‌سازی برای سگمنت بعدی
                context.user_data.pop("current_segment", None)
                return None
            else:
                # حالتی که کاربر بدون انتخاب شخصیت، متن وارد کرده است
                await update.message.reply_text(
                    "❌ لطفاً ابتدا یک شخصیت را از لیست انتخاب کنید.",
                    reply_markup=ReplyKeyboardMarkup([[f"🎭 شخصیت: {char['name']}" for char in context.user_data["story_characters"]][:2], 
                                                  ["✅ پایان ورود متن‌ها"],
                                                  ["🔙 برگشت"]], resize_keyboard=True)
                )
            ...
            logger.error(f"خطا در ورود متن داستان برای کاربر {user_id}: {str(e)}")
            await update.message.reply_text(
                "❌ خطایی در پردازش متن رخ داد. لطفاً دوباره تلاش کنید.",
                reply_markup=ReplyKeyboardMarkup([["🔙 برگشت"]], resize_keyboard=True)
            )
            return None
...
# تابع برای بررسی وضعیت صف پردازش
async def check_queue_status(user_id):
    """بررسی وضعیت صف پردازش برای کاربر"""
    # اگر صف برای کاربر وجود نداشته باشد، آن را ایجاد کن
    if user_id not in PROCESSING_QUEUE:
        PROCESSING_QUEUE[user_id] = []
        QUEUE_LOCK[user_id] = False
        return True
    
    # اگر صف خالی است یا قفل نیست، اجازه پردازش بده
    if not PROCESSING_QUEUE[user_id] or not QUEUE_LOCK[user_id]:
        return True
    
    # در غیر این صورت، کاربر باید منتظر بماند
    return False

# تابع برای اضافه کردن درخواست به صف
async def add_to_queue(update: Update, context: ContextTypes.DEFAULT_TYPE, process_type):
    """اضافه کردن درخواست به صف پردازش"""
    user_id = update.effective_user.id
    
    # ایجاد یک شناسه یکتا برای این درخواست
    request_id = f"{process_type}_{uuid4().hex}"
    ...
    PROCESSING_QUEUE[user_id].append(request_id)
    
    # اگر این اولین درخواست در صف است، قفل را فعال کن
    if len(PROCESSING_QUEUE[user_id]) == 1:
        QUEUE_LOCK[user_id] = True
        return request_id, 0  # بدون تاخیر
    
    # تعیین موقعیت در صف
    position = len(PROCESSING_QUEUE[user_id]) - 1
    
    # اطلاع‌رسانی به کاربر در مورد موقعیت در صف
    await update.message.reply_text(
        f"⏳ <b>درخواست شما در صف قرار گرفت</b>\n\n"
        f"موقعیت شما در صف: {position}\n"
        "لطفاً تا پردازش درخواست‌های قبلی صبر کنید.",
        parse_mode="HTML"
    )
    
    return request_id, position

# تابع برای حذف درخواست از صف
async def remove_from_queue(user_id, request_id):
    """حذف درخواست از صف پردازش و آزادسازی قفل"""
    if user_id in PROCESSING_QUEUE and PROCESSING_QUEUE[user_id]:
        if PROCESSING_QUEUE[user_id][0] == request_id:
            PROCESSING_QUEUE[user_id].pop(0)
            # اگر صف خالی نیست، قفل را فعال نگه دار
            QUEUE_LOCK[user_id] = bool(PROCESSING_QUEUE[user_id])
            return True
    return False

# تنظیمات داستان پیشرفته
MAX_CHARACTERS = 5  # حداکثر تعداد شخصیت‌ها
